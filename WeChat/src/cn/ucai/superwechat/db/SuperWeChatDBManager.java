...
public class SuperWeChatDBManager {
     ...
     * @param contactList
     */
    synchronized public void saveWeChatContactList(List<User> contactList) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            ...
            for (User user : contactList) {
                ...
                if (user.getMAvatarLastUpdateTime() != null)
                    values.put(UserDao.USER_COLUMN_AVATAR_UPDATE_TIEM, user.getMAvatarLastUpdateTime());
     ...
     * @return
     */
    synchronized public Map<String, User> getWeChatContactList() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                user.setMAvatarLastUpdateTime(cursor.getString(cursor.getColumnIndex(UserDao.USER_COLUMN_AVATAR_UPDATE_TIEM)));
     ...
     * @param username
     */
    synchronized public void deleteWeChatContact(String username) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
     ...
     * @param user
     */
    synchronized public void saveWeChatContact(User user) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(UserDao.USER_COLUMN_NAME, user.getMUserName());
        if (user.getMUserNick() != null)
